    ...
    if (df.empty == False):
        ...
        try:
            df["text"] = df.apply(lambda row:
                                  row["text"][1:] if (row["text"][0] != '.' and is_number(
                                      row["text"][0]) == False and is_number(row["text"][1:]) == True)
...
def mot_parasites(list):
    presence = False
    liste_parasite = ['change', 'charge', 'check', 'discout', 'fee', 'gratuity', 'guests', 'item', 'order', 'other',
                      'received',
